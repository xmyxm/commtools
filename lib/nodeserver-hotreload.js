//NODE开发过程中,每次改动都需要CTRL-C 然后再重启NODE进程, 所有长连接包括ZK连接和PEGION连接都需要重连,比较花费时间。 
//使用这个插件后, 自动监听/action文件夹下文件以及它们所依赖的模块文件(可以不在action文件夹内)的改动, 并通过删除模块缓存的方式进行热重载
//使用前需要添加依赖，配置文件需要替换，也可以自己写死，当前代码体现一个实现思路
"use strict"
var fs = require('fs');
